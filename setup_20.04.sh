...
# Description: Install and manage a Bradial Chat installation.
# OS: Ubuntu 20.04 LTS
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for Bradial Chat (e.g., chatwoot.domain.com): ' domain_name
...
serve Bradial Chat at https://$domain_name.
...
function setup_chatwoot() {
  ...
  git clone git@github.com:jhonnymoreno/bradialchat-v2.git chatwoot
  cd chatwoot
  git checkout "$BRANCH"
  bundle
  yarn
...
##############################################################################
# Setup Bradial Chat systemd services and cwctl CLI
# Globals:
#   None
# Arguments:
#   None
...
***************************************************************************
Woot! Woot!! Bradial Chat server installation is complete.
...
function cwctl_message() {
  echo $'\U0001F680 Try out the all new Bradial Chat CLI tool to manage your installation.'
...
***************************************************************************
              Bradial Chat Installation (v$CW_VERSION)
***************************************************************************
...
EOF
  ...
  sleep 3
  read -rp 'Would you like to configure a domain and SSL for Bradial Chat?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing Bradial Chat. This takes a long while."
...
function help() {
  ...
  cat <<EOF
Usage: cwctl [OPTION]...
Install and manage your Bradial Chat installation.
...
Installation/Upgrade:
  -i, --install             Install the latest stable version of Bradial Chat
  -I                        Install Bradial Chat from a git branch
  -u, --upgrade             Upgrade Bradial Chat to the latest stable version
...
Management:
  ...
  -l, --logs                View logs from Bradial Chat. Supported values include web/worker.
  -r, --restart             Restart Bradial Chat server
...
##############################################################################
# Get Bradial Chat web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function upgrade_redis() {
  ...
  echo "Upgrading Redis to v7+ for Rails 7 support(Bradial Chat v2.17+)"
...
function upgrade() {
  cwctl_upgrade_check
  get_cw_version
  echo "Upgrading Bradial Chat to v$CW_VERSION"
  sleep 3
  upgrade_prereq
  upgrade_redis
  upgrade_node
  ...
  # Navigate to the Bradial Chat directory
  cd chatwoot
...
##############################################################################
# Restart Bradial Chat server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
